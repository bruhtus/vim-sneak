...
func! sneak#to(op, input, inputlen, count, register, repeatmotion, reverse, inclusive, label) abort "{{{
  ...
  let bounds = a:repeatmotion ? g:st.bounds : [0,0] " [left_bound, right_bound]
  ...
  if !a:repeatmotion "this is a new (not repeat) invocation
    ...
    let g:st.rst = 0 | let g:st.input = a:input | let g:st.inputlen = a:inputlen
    let g:st.reverse = a:reverse | let g:st.bounds = bounds | let g:st.inclusive = a:inclusive
  ...
  if is_op && a:op !=# 'y'
    ...
    let g:st.label = target
    if empty(g:st.opfunc_st)
      let g:st.opfunc_st = filter(deepcopy(g:st), 'v:key !=# "opfunc_st"')
    endif
  endif
endf "}}}
...
func! sneak#wrap(op, inputlen, reverse, inclusive, label) abort
  ...
  let is_similar_invocation = a:inputlen == g:st.inputlen && a:inclusive == g:st.inclusive
  ...
  elseif a:op ==# 'g@' && !empty(g:st.opfunc_st) && !empty(g:st.opfunc) && g:st.opfunc ==# &operatorfunc
    " Replay state from the last 'operatorfunc'.
    call sneak#to(a:op, g:st.opfunc_st.input, g:st.opfunc_st.inputlen, cnt, reg, 1, g:st.opfunc_st.reverse, g:st.opfunc_st.inclusive, g:st.opfunc_st.label)
  else
...
func! sneak#rpt(op, reverse) abort
  if g:st.rst "reset by f/F/t/T
    exec "norm! ".(sneak#util#isvisualop(a:op) ? "gv" : "").v:count1.(a:reverse ? "," : ";")
    return
  endif

  let l:relative_reverse = (a:reverse && !g:st.reverse) || (!a:reverse && g:st.reverse)
  call sneak#to(a:op, g:st.input, g:st.inputlen, v:count1, v:register, 1,
        \ (g:sneak#opt.absolute_dir ? a:reverse : l:relative_reverse), g:st.inclusive, 1)
endf
